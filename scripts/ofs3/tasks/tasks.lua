...
function tasks.isTaskActive(name)
    for _, t in ipairs(tasksList) do
        if t.name == name then
            local age = ofs3.clock - t.last_run
            if name == "msp" then
...
function tasks.wakeup()
    ...
    local function canRunTask(task)
        ...
        local clockDelta = ofs3.clock - task.last_run
        ...
        return (not task.linkrequired or ofs3.session.telemetryState) and
            (priorityTask or overdue or not ofs3.app.triggers.mspBusy) and
