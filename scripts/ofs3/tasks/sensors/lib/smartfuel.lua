...
local function smartFuelCalc()
    ...
    if configSig ~= batteryConfigCache then
        ...
        resetVoltageTracking()
        stabilizeNotBefore = ofs3.clock + preStabiliseDelay -- start pre-stabilisation delay on config change
    end
   ...
   if stabilizeNotBefore and now < stabilizeNotBefore then
    ...
    if #lastVoltages >= 1 and ofs3.flightmode.current == "preflight" then
        ...
        if voltage > prev + voltageThreshold then
            ...
            fuelStartingConsumption = nil
            resetVoltageTracking()
            stabilizeNotBefore = ofs3.clock + preStabiliseDelay
    ...
    if consumption and fuelStartingConsumption and packCapacity > 0 then
        ...
        if not voltageStabilised or (stabilizeNotBefore and ofs3.clock < stabilizeNotBefore) then
