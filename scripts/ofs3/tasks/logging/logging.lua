...
local function checkLogdirExists()
        ...
        os.mkdir("LOGS:/ofs3/telemetry/" .. ofs3.session.mcu_id)
end
...
function logging.writeLogs(forcewrite)
    ...
    if #log_queue > 0 and logFileName then
        ofs3.utils.log("Write " .. #log_queue .. " lines to " .. logFileName,"info")
...
function logging.getLogLine()
    ...
    return os.date("%Y-%m-%d_%H:%M:%S") .. ", " .. ofs3.utils.joinTableItems(values, ", ")
end
...
function logging.flushLogs()
    if logFileName or logHeader then
        ofs3.utils.log("Flushing logs - ".. logFileName,"info")
...
function logging.wakeup()
    ...
    if ofs3.utils.inFlight() then
        if not logFileName then
            ...
            local iniName = "LOGS:ofs3/telemetry/" .. ofs3.session.mcu_id .. "/logs.ini"
