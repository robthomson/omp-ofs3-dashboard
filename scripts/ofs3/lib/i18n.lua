...
local function loadLangFile(filepath)
  local chunk, err = ofs3.compiler.loadfile(filepath)
...
function i18n.load(locale)
  ...
  if not translations then
    if ofs3.utils then -- cant log if utils is not available on startup
        ofs3.utils.log(
...
function i18n.get(key)
  ...
  if v ~= nil then
    --ofs3.utils.log("i18n: Cache hit for key: " .. key, "info")
    mark_hot(key)
    return v
  end
  ...
  --ofs3.utils.log("i18n: Cache miss for key: " .. key, "info")
  return v
end
