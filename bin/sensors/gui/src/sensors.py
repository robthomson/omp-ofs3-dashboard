...
class SensorApp:
    def __init__(self, root):
        ...
        self.rf_src = os.environ.get("FRSKY_OFS3_GIT_SRC")
        self.sim_src = os.environ.get("FRSKY_SIM_SRC")

        if not self.rf_src or not Path(self.rf_src).exists():
            raise EnvironmentError("FRSKY_OFS3_GIT_SRC is not set or invalid.")
        if not self.sim_src:
    ...
    def add_sensor_control(self, parent, sensor_elem):
        ...
        lua_path = self.sim_paths[0] / "ofs3" / "sim" / "sensors" / (name + SENSOR_FILE_EXT)
        control = None
