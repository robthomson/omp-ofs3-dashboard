...
FRSKY_OFS3_GIT_SRC = os.environ.get('FRSKY_OFS3_GIT_SRC')
...
def validate_paths():
    if not FRSKY_OFS3_GIT_SRC or not Path(FRSKY_OFS3_GIT_SRC).exists():
        print(f"[ERROR] FRSKY_OFS3_GIT_SRC not set or invalid: {FRSKY_OFS3_GIT_SRC}")
        sys.exit(1)
...
def parse_telemetry_lua():
    telemetry_file = Path(FRSKY_OFS3_GIT_SRC) / 'scripts' / 'ofs3' / 'tasks' / 'telemetry' / 'telemetry.lua'
...
def build_sensor_paths(sim_paths):
    for sensor in sensor_map.keys():
        for sim_base in sim_paths:
            sensor_file = Path(sim_base) / 'ofs3' / 'sim' / 'sensors' / f"{sensor}.lua"
